...
def download():
    try:
        ...
        if filename:
            print(f"ดาวน์โหลดเสร็จ: {filename}")  # Debugging
...
def download_youtube_audio(youtube_url):
    ...
    try:
        subprocess.run(command, check=False)  # บังคับให้รอจนกว่าการดาวน์โหลดเสร็จ
        downloaded_files = os.listdir(DOWNLOAD_FOLDER)
        if downloaded_files:
            print(f"ไฟล์ถูกดาวน์โหลดที่: {downloaded_files}")
            return downloaded_files[0]  # คืนชื่อไฟล์แรกที่พบ
        else:
            print("ไม่มีไฟล์ถูกดาวน์โหลด")
            return None
...
openai_api_key = "sk-2zA0NkEW6KyxW9M1ZRLBBUra4x8UREWWWESiUds_jzT3BlbkFJiURCiapbcVUJBnp9hmi7KwHBdi8GwEUslpqJmiWbwA"
...
def recommendation():
    ...
    try:
        ...
        classification_result = function_class_llm(subtitle)
...
def function_class_llm(subtitle):
